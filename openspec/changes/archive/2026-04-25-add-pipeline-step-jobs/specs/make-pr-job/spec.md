...
The system SHALL provide a `make-pr` job handled by an isolated Make PR module that owns deterministic repository finalization.
...
- **AND** use the received temporary repository path to finalize the issue branch as the terminal job in the requested pipeline
...
- **WHEN** Make PR completes or fails after receiving a temporary repository path
