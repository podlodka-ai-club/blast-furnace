...
export function getNodePtySpawnHelperPath() {
    ...
    try {
        ...
        return path.join(path.dirname(packageJsonPath), 'prebuilds', `${process.platform}-${process.arch}`, 'spawn-helper');
    }
    catch {
        return null;
    }
}
export async function ensureExecutable(filePath, logger) {
    try {
...
export async function ensureNodePtySpawnHelperExecutable(logger) {
