...
- не создавать run-level log file (`run.log` или замену);
...
```text
<blast-furnace repo>/.orchestrator/
  runs/
    <YYYY-MM-DD_HH.MM_runId>/
      <YYYY-MM-DD_HH.MM_runId>_run.json
      <YYYY-MM-DD_HH.MM_runId>_handoff.jsonl
```

Эта структура хранится в репозитории агента Blast Furnace, а не в cloned workspace целевого репозитория. Target workspace не должен получать `.orchestrator/**`.
...
- не payload очереди;
- не in-memory state;
- а JSONL handoff ledger внутри `<blast-furnace repo>/.orchestrator/runs/<YYYY-MM-DD_HH.MM_runId>/`.
...
## 4. Сохранить structured logging с привязкой к `runId` без run log files

### Что входит в работу

- использовать существующий structured logger;
- добавлять `runId` в события, где это помогает отладке;
- не писать `run.log`, run-level JSONL logs или заменяющие runtime log files в `.orchestrator/runs`.
...
- не добавлять runtime log files под `.orchestrator/runs`; события прогона должны оставаться в существующем structured logging или в будущих bounded event artifacts, если отдельная спецификация это введет.
...
7. Привязать существующие structured logs к `runId` без run-level log files.
...
- `run.json` отражает реальное состояние прогона;
- structured logs и handoff records позволяют отследить `runId` без run-level log files;
- `clarify / rework` существуют как часть state machine;
- GitHub-visible side effects определены и встроены в workflow.

## Дополнительные задачи после verification `run-handoff-jsonl-contracts`

Эти пункты найдены при verification изменения `run-handoff-jsonl-contracts`. Сейчас они не блокируют текущую реализацию, но их нужно отдельно разобрать перед закреплением handoff/queue контрактов как стабильных.

### Проверить границу downstream queue payload и handoff record validation

Валидацию downstream queue payload сейчас оставляем как есть, но нужно отдельно посмотреть на связку queue payload и handoff record schema validation как на один контракт.

Найденные проблемы:

- downstream queue payload validation недостаточно строгая. `StageJobPayload` все еще наследует optional `payload?: Record<string, unknown>`, а `validateStagePayload` запрещает только известные business-поля верхнего уровня. Поэтому downstream payload может пронести дублирующее business-состояние внутри `payload` или другого неизвестного ключа и все равно пройти проверку. Также есть тест, который все еще проверяет, что `scheduleNextJob` пропускает business-heavy payload без изменений;
- handoff record schema validation слишком поверхностная для требования "full handoff record". `validateHandoffRecord` не проверяет согласованность `toStage` и `nextInput`, а `nextInput` типизирован как `StageJobPayload | null`, хотя фактически туда кладется payload с `inputRecordRef` через cast.

Что нужно сделать отдельно:

- решить, должен ли downstream queue payload быть exact-shape allow-list или текущей проверки известных business-полей достаточно;
- проверить, не может ли optional `payload` или неизвестный ключ снова стать скрытым handoff-каналом;
- проверить, что `nextInput` в JSONL не расходится с `toStage`, `runId`, `stageAttempt`, `reworkAttempt`, `recordId` и `sequence`;
- решить, нужна ли более точная типизация `nextInput` под реальную downstream payload/ref форму;
- обновить тесты так, чтобы они защищали выбранный контракт, а не старую business-rich payload модель.

Критерий готовности: понятно и зафиксировано, где проходит граница между BullMQ transport payload, `nextInput` в handoff record и durable business-state в JSONL ledger.

### Проверить накопление контекста в JSONL handoff records

Handoff JSONL records сейчас работают как накопительные snapshots контекста: каждый следующий stage output включает часть данных предыдущих этапов. Сейчас с этим ничего не делаем, но нужно отдельно проверить, что именно сохраняется при handoff и зачем.

Что нужно сделать отдельно:

- пройтись по каждому этапу и определить, какие поля он должен записывать в свой handoff output;
- проверить, какие этапы не должны ничего накапливать сверх своего собственного результата;
- определить, какие этапы действительно должны сохранять расширенный контекст для downstream-потребителей;
- для этапов с накоплением сделать ручную проверку и явно решить, в каком случае сохранять context snapshot в handoff;
- решить, должен ли consumer читать один последний snapshot или проходить по `dependsOn` chain и собирать нужный контекст из нескольких записей;
- обновить output schemas и тесты под выбранную модель.

Критерий готовности: для каждого stage output понятно, является ли он stage-local результатом или намеренным context snapshot, и JSONL ledger не дублирует данные случайно.
